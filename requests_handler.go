...
func getXAxisLabels() string {
	return `function (value) {
	let dateAsValue = new Date(value);
...
func generateGraphOfMensaTrend() (string, error) {
	...
	queueLengthsAsStringSlice, timestampsSlice, err := GetAllQueueLengthReportsInTimeframe(graphTimeFrameInSeconds)
	...
	if len(timestampsSlice) < 3 {
		return "", errors.New("Not enough data in timeframe")
	}

	// TODO find out how to do our pretty labels

	// Generate labels:
	// 5 mins ago, 10 mins ago, ... 30 mins ago
	// But just the time

	line := charts.NewLine()
	// TODO xAxis. max to get the current time in there
	...
	line.SetGlobalOptions(
		...
		charts.WithXAxisOpts(opts.XAxis{
			// Type: "time", // TODO this breaks stuff, but without it we get literal HH:MM
			// FML it's a library bug
			AxisLabel: &opts.AxisLabel{
	...
	//yData := []int{3, 4, 2}
	xData := timesSlice
	yData := queueLengthsSlice
	/*xData := []time.Time{
		currentTime.Add(time.Minute * 5),
		currentTime.Add(time.Minute * 10),
		currentTime.Add(time.Minute * 15)}
	/*
		xLabelTimes = []time.Time{currentTime,
			currentTime.Sub(time.Minute * 5),
			currentTime.Sub(time.Minute * 10),
			currentTime.Sub(time.Minute * 15),
			currentTime.Sub(time.Minute * 20),
			currentTime.Sub(time.Minute * 25)}
	*/
	...
	return "/home/antonio/IT/go/mensa_queue_bot/profile_picture.jpg", nil
...
func GenerateAndSendGraphicQueueLengthReport(chatID int) {
	...
	_, err := generateGraphOfMensaTrend()
	zap.S().Debug("NEW GRAPH GENERATED!")
	if err != nil {
		return
	}
